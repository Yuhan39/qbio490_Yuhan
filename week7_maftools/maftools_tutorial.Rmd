...
young_patient_barcodes <- maf_object@clinical.data$Tumor_Sample_Barcode[young_mask]# FILL IN HERE
```
...
young_maf <- subsetMaf(maf = maf_object,
                       tsb = young_patient_barcode)
```

Create an old_patient_barcode vector and use that to subset just the old patient data.

```{r Exercise 4.3}
old_patient_barcodes <-ifelse(clinical.data$age_category=="Old",T,F)# FILL IN HERE
  
old_maf <- subsetMaf(maf = maf_object,
                       tsb =old_patient_barcode)
```
...
```{r Exercise 9.1}
maf_obejct@clinical.data$survival_time,-ifelse(maf_object@clinical.data$vital_status=="Dead",)
