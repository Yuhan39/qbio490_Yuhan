...
write.csv(clinical, "/home1/yuhanyua/490_cluster/analysis_data/brca_clinical_data.csv", row.names = FALSE)
```
...
Notice the @ and $ signs. What do each of them represent?
  ...
  @ represents:accessing the slots of an S4 object
  $ represents:accessing column/variables in a dataframe
...
```{r Exercise 3.4}
# write code here
maf_object@data[1:110,maf_object@data$Tumor_Sample_Barcode]
maf_object@clinical.data[1:110,maf_object@clinical.data$Tumor_Sample_Barcode]
...
```{r Exercise 3.5}
# write code here
TP53mask<-ifelse(maf_object@data$Hugo_Symbol=="TP53",TRUE, FALSE)
maf_object@data[TP53mask,c("Tumor_Sample_Barcode","HGVSp_Short")]
```
...
Refer to a codon chart and amino acid abbreviation chart to answer the following:
 ...
 The third nucleotides in codon changes from U or A to C or G
 - What about an A/T change?
 The first nucleotide in this code changes from G to A
...
protein amino acids proline in position728 changes to Alanine which causes a frameshift with a new stop codon at 15 amino acids after Alanine
