---
title: "Intro to TCGA Clinical Data_completed"
date: 07/17/22
updated: 09/26/24
---
...
```{r}
write.csv(clinic, "/home1/yuhanyua/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/yuhanyua/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
str(clinic)
head(clinic)
#1.there are 1097 rows and 112 columns
#2.character
clinic[5,c("age_at_diagnosis","last_contact_days_to")]
age<-50+158/365
age
#3.approximately 50 years old
colnames(clinic)
clinic[1,c("age_at_diagnosis","last_contact_days_to")]
age_at_last_followup<-55+3767/365
age_at_last_followup
#4.the age at last followup is appriximately 65
```
...
plot(clinic$age_at_diagnosis , clinic$lymph_nodes_examined_count,
     main = "Scatter Plot of Lymph Nodes Examined Count with respect to Age at Diagnosis",
     xlab="age_at_aiagnosis", ylab ="lymph_nodes_examined_count",
     col="green")
...
```{r Exercise 4.2}
# create plot here
clinic$lymph_nodes_examined_count<-as.numeric(as.character(clinic$lymph_nodes_examined_count))
#the data type for lymph_nodes_examined_count is char using str(clinic), so first converted to numeric
boxplot(lymph_nodes_examined_count~gender,
        data=clinic,
        main = "Boxplot of Gender and Lymph Nodes Examined",
        xlab="gender", ylab = "Lymph Nodes Examined Count", 
        col = c("blue", "green"))
```
...
```{r}
unlink('/home1/yuhanyua/R/x86_64-pc-linux-gnu-library/4.4/00LOCK-doBy', recursive = TRUE)
...
cleaned_clinic$age_category <- ifelse(cleaned_clinic$age_at_diagnosis<=35,"young",
                                      ifelse(cleaned_clinic$age_at_diagnosis<50,
                                             "middle","old")
                                      )
head(cleaned_clinic)
...
survival_object <- Surv(time = cleaned_clinic$survival_time,
                        event = as.numeric(cleaned_clinic$death_event=="T"))
...
p=0.2. Not significant, indicating that there is no statistically significant relationship between age and breast cancer survival in this context.
