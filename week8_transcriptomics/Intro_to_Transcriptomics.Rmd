...
rna_counts <- read.csv("BCRA_rna_counts.csv", row.name=1)
```
...
```{r}
#Variable of interest:tumor or normal tissue
rna_clinical$definiton<-factor(rna_clinical$definition)

#covariates controlled: age
...
na_mask<-!is.na(rna_clinical$age_at_diagnosis)&!is.na(rna_clinical$vital_status)&!is.na(rna_clinical$definition)
...
class(rna_clinical$definition)

#pre-filtering

#str(rna_counts)
#gene_filter <- rowSums(rna_counts[ ,-1, drop = FALSE]) >= 1000
#select rows except the first colume 'X' because rows are genes identifiers and they are character not numeric
#drop=False ensures that the result remains a data frame
gene_filter <- rowSums(rna_counts) >= 1000
filtered_rna_counts <- rna_counts[gene_filter, ]
...
dds <- DESeqDataSetFromMatrix(countData = rna_counts,
                              ...
                              design=~age_at_diagnosis+vital_status+definition)
...
ggsave("/home1/yuhanyua/490_cluster/week8_transcriptomics/EnhancedVolcano_BRCA_TumorVSNormal.png")
```

*5*
# Explain what genes from each part of the Volcano Plot mean in terms of their significance and up/down regulation. 
top-right genes: significantly upregulated in tumor compared to normal tissue
bottom-right genes: is not significantly upregulated in tumor compared to normal tissue
top-left genes: is significantly down regulated in tumor compared to normal tissue
bottom-left genes: is not significantly down regulated in tumor compared to normal tissue
top-middle genes: with modest fold changes and high significance.
bottom-middle genes: with modest fold changes and low significance
