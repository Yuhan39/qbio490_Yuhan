...
1. survival analysis between MUC16 mutated samples and MUC16 non-mutated samples
```{r}
...
MUC16_mutated_samples <- maf_object@data[maf_object@data$Hugo_Symbol == "MUC16", "Tumor_Sample_Barcode"]
maf_clinical<-maf_object@clinical.data
maf_clinical$MUC16_mutation_status<-ifelse(maf_clinical$Tumor_Sample_Barcode%in%MUC16_mutated_samples$Tumor_Sample_Barcode,T,F)
```
...
fit_object <- survfit(survival_object ~ MUC16_mutation_status, data = maf_clinical)
...
KM_plot
#ggsave("/home1/yuhanyua/490_cluster/final_project/KMplot_mutated_VS_non-mutated.png")
```

2, co-oncoplot using MAF data to show top n most commonly mutated genes in MUC16 mutated samples compared to MUC16 non-mutated samples. This could tell a possible correlation(not causation) between MUC16 mutation and other genes that the mutation of which may possibly involve in tumor progression
...
MUC16_barcodes<-maf_clinical$Tumor_Sample_Barcode[maf_clinical$MUC16_mutation_status==TRUE]
non_MUC16_barcodes<-maf_clinical$Tumor_Sample_Barcode[maf_clinical$MUC16_mutation_status==FALSE]


MUC16_maf <- subsetMaf(maf = maf_object,
                       tsb = MUC16_barcodes)
non_MUC16_maf <- subsetMaf(maf = maf_object,
                       tsb = non_MUC16_barcodes)

m1.genes = getGeneSummary(MUC16_maf)[1:5]
m2.genes = getGeneSummary(non_MUC16_maf)[1:5]

MUC16_maf.genes <- getGeneSummary(MUC16_maf)[, .(Hugo_Symbol, MutatedSamples)]
non_MUC16_maf.genes <- getGeneSummary(non_MUC16_maf)[, .(Hugo_Symbol, MutatedSamples)]
mdt <- merge(MUC16_maf.genes, non_MUC16_maf.genes, by = "Hugo_Symbol", all = TRUE)
...
#png("/home1/yuhanyua/490_cluster/final_project/coOncoplot_mutated_VS_non-mutated.png", width = 1000, height = 800)

co_oncoplot<-coOncoplot(m1 = MUC16_maf, 
           m2 = non_MUC16_maf, 
           m1Name = "MUC16 mutated", 
           m2Name = "MUC16 non-mutated", 
           borderCol = NA
           )
...
3. Differential expression analysis using RNA-seq data. This will visualize the upregulation and downregulation of genes in MUC16 mutated samples compared to non-mutated patients, showing correlation between MUC16 mutation and other genes' expression which may involved in tumor progression.
...
rna_clinical$MUC16_mutation_status<-ifelse(rna_clinical$patient%in%MUC16_mutated_samples$Tumor_Sample_Barcode,T,F)
rna_clinical$MUC16_mutation_status<-factor(rna_clinical$MUC16_mutation_status)
...
dds <- DESeqDataSetFromMatrix(countData = rna_counts,
                            ...
                            design=~race+Radiation+Chemo+Immuno+Molecular+Vaccine+MUC16_mutation_status)
...
results <- results(dds_obj,
                   format = "DataFrame", 
                   contrast = c("MUC16_mutation_status", "TRUE", "FALSE"))
...
#                pCutoff = 0.05,
#                FCcutoff = 1, 
#                selectLab = c("NRAS", "LRP1B", "PKHD1L1", "BRAF"),
...
EnhancedVolcano(results,
                ...
                xlim = c(-5, 5),  
                ylim = c(0, 25),  
                selectLab = c("TTH", "LRP1B", "PKHD1L1", "BRAF"),  # Select specific genes
                title = 'Gene expression: MUC16 mutated samples VS MUC16 non-mutated samples',
                pointSize = 1.0,
                labSize = 5.0,
                drawConnectors = TRUE,
...
#results[results$gene_name %in% c("NRAS", "LRP1B", "PKHD1L1", "BRAF"), ]
...
4.Boxplot to compare MUC16 transcriptional expression between metastatic and non-metastatic samples using transcriptomics data.
```{r}
...
```
```{r}
#read in methylation data, this should be comment out after submitting
...
```{r}
gene_counts_mask <- rna_genes$gene_name == "MUC16"
gene_betas_mask <- cpg_sites$gene == "MUC16"
...
#use R base plotting to save the box plot
#png("/home1/yuhanyua/490_cluster/final_project/boxplot_MUC16transcriptomics_MetastaticVSnon-metastatic.png", width=800, height=600)
...
boxplot(rna_nonMetastatic, rna_metastatic,
        ...
        ylim = c(0, 500))          #setting the boundary for y-axis(counts)

#save the boxplot
#dev.off()

#little difference is found
#MUC16’s role in metastasis is well-documented in ovarian cancer and other cancers, but its role in melanoma metastasis may be different and might require different molecular context or co-factors to be significant.
```


5. bargraph to compare methylation levels across CpG sites of MUC16 gene in metastatic versus non-metastatic samples, highlighting potential epigenetic regulation.
```{r}
#png("/home1/yuhanyua/490_cluster/final_project/barplot_MUC16_methylation_MetastaticVSnon-metastatic.png", width=800, height=600)
